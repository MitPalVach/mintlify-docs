---
title: starting vector generation
description: starting vector serves as the starting point for gsm (global state machine) initialization and also acts as the basis for key generation.
---

generation of $\textbf{SV}$ proceeds by summing products of field elements $\{p_{ij}\}$ with coefficients $a_{ij}$, after transformation via functions $\phi_i$:

$$
\textbf{SV}_i = \sum_{j=1}^{m_i} a_{ij} \cdot \phi_i(p_{ij})
$$

this process assembles $\textbf{SV}$, essential for cryptographic key generation.
